...
if uploaded_file and st.button("Execute Translation Blueprint"):
    ...
    # Parse source PDF using pypdf
    reader = PdfReader(uploaded_file)
    total_pages = len(reader.pages)
    
    # Instantiate blank Word document skeleton
    doc = Document()
    
    # Setup interactive UI indicators
    progress_bar = st.progress(0)
    status_msg = st.empty()
    
    # Tracking variables for cross-page semantic context tracking
    ...
    for idx in range(total_pages):
        ...
        # Isolate text content of current page frame
        current_page_text = reader.pages[idx].extract_text() or "[Blank Page Text]"
        
        # Define advanced system instruction prompt with Strict Layout Boundaries
        prompt = f"""
        ...
        # Request Structured JSON Output from Gemini 2.5 Flash
        ...
        # Safely parse the structured JSON payload back into Python objects
        try:
            translation_data = PageLayoutTranslation.model_validate_json(response.text)
        except Exception as e:
            # Fallback handling if JSON structure encounters an edge-case parse error
            st.warning(f"Formatting anomaly on Page {idx + 1}. Running standard extraction fallback.")
            fallback_text = response.text
            translation_data = PageLayoutTranslation(
                blocks=[ContentBlock(block_type="paragraph", text=fallback_text)]
            )
        
        # Save the current output string as the background context history record for the next page
        previous_page_context = f"The previous page ended with this translation text: " + " ".join([b.text for b in translation_data.blocks[-2:]])
        ...
        for block in translation_data.blocks:
            ...
            if b_type == "heading_1":
                ...
                # Intercept footer page elements, align right, style bold crisp navy-blue
                p = doc.add_paragraph()
        ...
        # Inject an explicit page break inside the Word doc if more sections follow
        if idx < total_pages - 1:
            doc.add_page_break()
            
        # Update progressive visual rendering benchmarks
        progress_bar.progress((idx + 1) / total_pages)
        time.sleep(0.5)  # Smooth processing pacing
        
    status_msg.success("All pages parsed and compiled successfully!")
    
    # Write structural object layout out to an in-memory data stream buffer
    docx_buffer = io.BytesIO()
    doc.save(docx_buffer)
    docx_buffer.seek(0)
    
    # Save the completed binary data array into memory for persistent cloud rendering
...
if st.session_state.translated_docx_bytes is not None:
    ...
    # Safely deliver file object out of cloud sandbox boundaries without button resets
    st.download_button(
